...
async function insertMonitor(req, res, next) {
  ...
  if (alreadyExists)
    return res.status(409).send(`Already exists a monitor with these params.`);
  ...
  if (savedMonitor.isActive) {
    ...
    // appEm.startChartMonitor(
    //   savedMonitor.symbol,
    //   savedMonitor.interval,
    //   indexes,
    //   savedMonitor.broadcastLabel,
    //   savedMonitor.logs,
    // );
  }
...
async function updateMonitor(req, res, next) {
  ...
  const indexes = updatedMonitor.indexes
    ? updatedMonitor.indexes.split(',')
    : [];

  // if (updatedMonitor.isActive) {
  //   appEm.stopChartMonitor(
  //     currentMonitor.symbol,
  //     currentMonitor.interval,
  //     oldIndexes,
  //     currentMonitor.logs,
  //   );
  //   appEm.startChartMonitor(
  //     updatedMonitor.symbol,
  //     updatedMonitor.interval,
  //     indexes,
  //     updatedMonitor.broadcastLabel,
  //     updatedMonitor.logs,
  //   );
  // } else {
  //   appEm.stopChartMonitor(
...
async function deleteMonitor(req, res, next) {
  ...
  if (currentMonitor.isActive) {
    // const indexes = currentMonitor.indexes
    //   ? currentMonitor.indexes.split(',')
    //   : [];
    // appEm.stopChartMonitor(
    //   currentMonitor.symbol,
